...
void sort() {
    ...
    for (int i = 0; i < studentCount; ++i) {
        fread(&student[i],sizeof (struct account),1,fptr);
    }
...
void updateBinary(long arrayOffset,int offset,int score)
{
    ...
    fwrite(&student[arrayOffset], sizeof (struct account),1,fptr);
    fclose(fptr);
}
updateScore(){
    struct binOffset temp[studentCount];
    ...
    long last = studentCount-1;
    long middle = (first + last) / 2;
    printf("Enter the number you want to update its score : ");
    scanf("%ld", &userID_input);
    FILE *fptr;
    if ((fptr = fopen("indexFile.txt", "r")) == NULL) {
        printf("Error! opening file");
    ...
    for (int i = 0; i < studentCount; ++i) {
        fscanf(fptr, "%d %ld", &temp[i].offset, &temp[i].studentID);
    }
    fclose(fptr);
    int newscore;
...
deleteStudent(){
    ...
    while (first <= last) {
        if (temp[middle].studentID > userID_input) {
            ...
            printf("\nThe %ld student has removed from the server!\n",userID_input);       //??????????????
        } else
            last = middle - 1;
        middle = (first + last) / 2;
    }
...
int main() {
    ...
    while(option!=0) {
        ...
        printf("Option 8: Update the student's Score...\n");
        printf("Option 9: Delete the student...\n\n");
        ...
        switch (option) {
            ...
            {
                addStudent();
                printf("\nOperation of adding student has completed!\n");
                break;
            }
            case 8:     //Update the student score.
            {
                updateScore();
                printf("Update operation has finished ! ");
                break;
            }
